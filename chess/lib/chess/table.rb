...
class Table
	def initailize 
		...
		@height = 8
		@size = @width * @height
		@fields = Array.new(@size).map_with_index do |_, i|
	...
	def get_field_where_king_of(color)
		@fields.select { |f| f.figure.is_a?(King) && f.figure.color == color }
	end
	...
	def rising_diagonal_path_clear?(coords1, coords2)
 	 figs = get_figures(coords1.get_rising_path(coords2))
 	 path_clear(figs)
 	end

 	def falling_diagonal_path_clear?(coords1, coords2)
 	 figs = get_figures(coords1.get_falling_path(coords2))
 	 path_clear(figs)
 	end
	...
	def index.index_to_chess_coords(index)
		x = index % @width
